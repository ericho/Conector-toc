...
class gui(object):
    ...
    def llenar_campos_modelo(self):
        ...
        self.mod1_temp_mezcla = self.treestore.append(self.modulo_1, ['Temperatura mezcla', '-- °C'])
        self.mod1_temp_reactor = self.treestore.append(self.modulo_1, ['Temperatura reactor', '-- °C'])
        ...
        self.modulo_2 = self.treestore.append(None, ['Biodigestor Bioetanol', '--'])
        self.mod2_presion_reactor = self.treestore.append(self.modulo_2, ['Presión reactor', '-- ps'])
        self.mod2_presion_contenedor = self.treestore.append(self.modulo_2, ['Presión contenedor', '-- ps'])
        self.mod2_presion_3 = self.treestore.append(self.modulo_2, ['Presión 3', '-- ps'])
        self.mod2_temp_contenedor = self.treestore.append(self.modulo_2, ['Temperatura contenedor', '-- °C'])
        self.mod2_temperatura_reactor = self.treestore.append(self.modulo_2, ['Temperatura reactor', '-- °C'])
        self.mod2_temperatura_almacenaje = self.treestore.append(self.modulo_2, ['Temperatura almacenaje', '-- °C'])
        self.mod2_nivel_mezcla_reactor = self.treestore.append(self.modulo_2, ['Nivel mezcla reactor', '-- %'])
        self.mod2_nivel_condensador = self.treestore.append(self.modulo_2, ['Nivel condensador', '-- %'])
        
        self.modulo_3 = self.treestore.append(None, ['Reactor biodiesel', '--'])
        self.mod3_temp_reactor = self.treestore.append(self.modulo_3, ['Temperatura reactor', '-- °C'])
        ...
        self.mod3_res_calentador_estado = self.treestore.append(self.modulo_3, ['Resistencia calentador estado', '--'])
        ...
        self.mod4_temp_agua_fria = self.treestore.append(self.modulo_4, ['Temperatura agua fria', '-- °C'])
        self.mod4_temp_agua_caliente = self.treestore.append(self.modulo_4, ['Temperatura agua caliente', '-- °C'])
        self.mod4_temp_salida_agua_caliente = self.treestore.append(self.modulo_4, ['Temperatura salida agua caliente', '-- °C'])
        self.mod4_temp_tuberia = self.treestore.append(self.modulo_4, ['Temperatura tuberia', '-- °C'])
        ...
        self.mod5_voltaje = self.treestore.append(self.modulo_5, ['Voltaje', '-- V'])
        self.mod5_velocidad_viento = self.treestore.append(self.modulo_5, ['Velocidad viento', '--'])
        self.mod5_potencia = self.treestore.append(self.modulo_5, ['Potencia', '-- W'])
        self.mod5_rpm = self.treestore.append(self.modulo_5, ['RPM', '-- rpm'])
        
        self.modulo_6 = self.treestore.append(None, ['Generador magnetico', '--'])
        self.mod6_corriente = self.treestore.append(self.modulo_6, ['Corriente', '-- A'])
        self.mod6_voltaje = self.treestore.append(self.modulo_6, ['Voltaje', '-- V'])
        self.mod6_rpm = self.treestore.append(self.modulo_6, ['RPM', '-- rpm'])
        
        
        self.modulo_7 = self.treestore.append(None, ['Generador calentador Stirling', '--'])
        self.mod7_temp_caliente = self.treestore.append(self.modulo_7, ['Temperatura entrada caliente', '-- °C'])
        self.mod7_temp_fria = self.treestore.append(self.modulo_7, ['Temperatura entrada fria', '-- °C'])
        self.mod7_rpm = self.treestore.append(self.modulo_7, ['RPM', '-- rpm'])
        self.mod7_presion = self.treestore.append(self.modulo_7, ['Presion', '-- ps'])
        ...
        self.mod9_motor_frecuencia = self.treestore.append(self.modulo_9, ['Motor frecuencia', '--'])
        self.mod9_humedad = self.treestore.append(self.modulo_9, ['Humedad', '--'])
        self.mod9_temperatura = self.treestore.append(self.modulo_9, ['Temperatura', '-- °C'])
        
        self.modulo_10 = self.treestore.append(None, ['Acuaponia', '--'])
        self.mod10_temp_ambiente = self.treestore.append(self.modulo_10, ['Temperatura ambiente', '-- °C'])
        self.mod10_temp_agua = self.treestore.append(self.modulo_10, ['Temperatura agua', '-- °C'])
        ...
        self.mod11_temp_sol = self.treestore.append(self.modulo_11, ['Temperatura Sol', '-- °C'])
        self.mod11_temp_lente = self.treestore.append(self.modulo_11, ['Temperatura Lente', '-- °C'])
        self.mod11_temp_interna = self.treestore.append(self.modulo_11, ['Temperatura Interna', '-- °C'])
        ...
        self.mod12_temp_ambiente = self.treestore.append(self.modulo_12, ['Temperatura ambiente', '-- °C'])
        self.mod12_temp_interior = self.treestore.append(self.modulo_12, ['Temperatura interior', '-- °C'])
        self.mod12_temp_agua = self.treestore.append(self.modulo_12, ['Temperatura agua', '-- °C'])
        self.mod12_humedad = self.treestore.append(self.modulo_12, ['Humedad', '-- %'])
        ...
        self.mod12_flujo = self.treestore.append(self.modulo_12, ['Flujo de agua', '--'])
        ...
        self.mod13_bomba_3 = self.treestore.append(self.modulo_13, ['Bomba 3', '--'])
        
        self.modulo_14 = self.treestore.append(None, ['Autonomia de transporte', '--'])
        self.mod14_inclinacion = self.treestore.append(self.modulo_14, ['Inclinación', '--'])
        self.mod14_rpm = self.treestore.append(self.modulo_14, ['RPM', '-- rpm'])
        self.mod14_alternador = self.treestore.append(self.modulo_14, ['Alternador', '--'])
        
        self.modulo_15 = self.treestore.append(None, ['Bomba de calor', '--'])
        self.mod15_presion = self.treestore.append(self.modulo_15, ['Presion 1', '--'])
        self.mod15_presion_domo = self.treestore.append(self.modulo_15, ['Presion domo', '--'])
    ...
    def actualizar_modelo(self, trama):
        if trama[1] == "01":
            ...
            self.treestore.set(self.mod1_temp_mezcla, 1, trama[2] + " °C")
            self.treestore.set(self.mod1_temp_reactor, 1, trama[3] + " °C")
            ...
            self.treestore.set(self.mod2_presion_reactor, 1, trama[2] + " ps")
            self.treestore.set(self.mod2_presion_contenedor, 1, trama[3] + " ps")
            self.treestore.set(self.mod2_presion_3, 1, trama[4] + " ps")
            self.treestore.set(self.mod2_temp_contenedor, 1, trama[5] + " °C")
            self.treestore.set(self.mod2_temperatura_reactor, 1, trama[6] + " °C")
            self.treestore.set(self.mod2_temperatura_almacenaje, 1, trama[7] + " °C")
            self.treestore.set(self.mod2_nivel_mezcla_reactor, 1, trama[8])
            self.treestore.set(self.mod2_nivel_condensador, 1, trama[9])
            
        elif trama[1] == '03':
            self.treestore.set(self.modulo_3, 1, "Activo")
            self.treestore.set(self.mod3_temp_reactor, 1, trama[2] + " °C")
            ...
            self.treestore.set(self.mod4_temp_agua_fria, 1, trama[2] + " °C")
            self.treestore.set(self.mod4_temp_agua_caliente, 1, trama[3] + " °C")
            self.treestore.set(self.mod4_temp_salida_agua_caliente, 1, trama[4] + " °C")
            self.treestore.set(self.mod4_temp_tuberia, 1, trama[5] + " °C")
            ...
            self.treestore.set(self.mod5_voltaje, 1, trama[2] + " V")
            self.treestore.set(self.mod5_velocidad_viento, 1, trama[3])
            self.treestore.set(self.mod5_potencia, 1, trama[4] + " W")
            self.treestore.set(self.mod5_rpm, 1, trama[5] + " rpm")
            
        elif trama[1] == "06":
            self.treestore.set(self.modulo_6, 1, "Activo")
            self.treestore.set(self.mod6_corriente, 1, trama[2] + " A")
            self.treestore.set(self.mod6_voltaje, 1, trama[3] + " V")
            self.treestore.set(self.mod6_rpm, 1, trama[4] + " rpm")
            
        elif trama[1] == "07":
            self.treestore.set(self.modulo_7, 1, "Activo")
            self.treestore.set(self.mod7_temp_caliente, 1, trama[2] + " °C")
            self.treestore.set(self.mod7_temp_fria, 1, trama[3] + " °C")
            self.treestore.set(self.mod7_rpm, 1, trama[4] + " rpm")
            self.treestore.set(self.mod7_presion, 1, trama[5] + " ps")
            
        elif trama[1] == "08":
            self.treestore.set(self.modulo_8, 1, "No recibido")
        
        elif trama[1] == "09":
            self.treestore.set(self.modulo_9, 1, "Activo")
            self.treestore.set(self.mod9_motor_estado, 1, trama[2])
            self.treestore.set(self.mod9_motor_frecuencia, 1, trama[3] + " Hz")
            self.treestore.set(self.mod9_humedad, 1, trama[4] + " %")
            self.treestore.set(self.mod9_temperatura, 1, trama[5] + " °C")
        
        elif trama[1] == "10":
            self.treestore.set(self.modulo_10, 1, "Activo")
            self.treestore.set(self.mod10_temp_ambiente, 1, trama[2] + " °C")
            self.treestore.set(self.mod10_temp_agua, 1, trama[3] + " °C")
            ...
            self.treestore.set(self.mod11_temp_sol, 1, trama[2] + " °C")
            self.treestore.set(self.mod11_temp_lente, 1, trama[3] + " °C")
            self.treestore.set(self.mod11_temp_interna, 1, trama[4] + " °C")
            self.treestore.set(self.mod11_nivel_contenedor, 1, trama[5])
            
        elif trama[1] == "12":
            self.treestore.set(self.modulo_12, 1, "Activo")
            self.treestore.set(self.mod12_temp_ambiente, 1, trama[2] + " °C")
            self.treestore.set(self.mod12_temp_interior, 1, trama[3] + " °C")
            self.treestore.set(self.mod12_temp_agua, 1, trama[4] + " °C")
            self.treestore.set(self.mod12_humedad, 1, trama[5] + " %")
            self.treestore.set(self.mod12_ldr, 1, trama[6])
            self.treestore.set(self.mod12_motor, 1, trama[7])
            self.treestore.set(self.mod12_flujo, 1, trama[8])
            ...
            self.treestore.set(self.mod13_bomba_1, 1, trama[5])
            self.treestore.set(self.mod13_bomba_2, 1, trama[6])
            self.treestore.set(self.mod13_bomba_3, 1, trama[7])
            
        elif trama[1] == "14":
            self.treestore.set(self.modulo_14, 1, "Activo")
            self.treestore.set(self.mod14_inclinacion, 1, trama[2])
            self.treestore.set(self.mod14_rpm, 1, trama[3])
            self.treestore.set(self.mod14_alternador, 1, trama[4])
            
        elif trama[1] == "15":
            self.treestore.set(self.modulo_15, 1, "Activo")
            self.treestore.set(self.mod15_presion, 1, trama[2] + " ps")
            self.treestore.set(self.mod15_presion_domo, 1, trama[3] + " ps")
            self.treestore.set(self.mod15_temp_fria, 1, trama[4] + " °C")
            self.treestore.set(self.mod15_temp_caliente, 1, trama[5] + " °C")
            self.treestore.set(self.mod15_temp_sal_caliente, 1, trama[6] + " °C")
            self.treestore.set(self.mod15_temp_tuberia, 1, trama[7] + " °C")
